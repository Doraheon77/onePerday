        ...
        <>
                  ...
                  <tr key={s.id} style={{ borderTop: '1px solid #f0f0f0', backgroundColor: i % 2 === 0 ? 'white' : '#fafff8' }}>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <button
                        ...
                        style={{ backgroundColor: '#fff0f0', color: '#d32f2f', border: '1px solid #ffcdd2', borderRadius: '8px', padding: '6px 14px', fontSize: '13px', cursor: 'pointer', fontWeight: 600 }}
                      >
                        삭제
                      </button>
                    </td>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '6px', marginTop: '1.5rem' }}>
            <button
              ...
              disabled={page === 1}
              style={{ padding: '8px 14px', borderRadius: '8px', border: '1.5px solid #c8e6c9', backgroundColor: 'white', cursor: page === 1 ? 'not-allowed' : 'pointer', color: page === 1 ? '#aaa' : '#2d8a5e', fontWeight: 600 }}
            >
              이전
            </button>
              ...
              .map((p, idx, arr) => (
                <div key={p} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  ...
                  <button
                    ...
                    style={{ padding: '8px 14px', borderRadius: '8px', border: `1.5px solid ${page === p ? '#2d8a5e' : '#c8e6c9'}`, backgroundColor: page === p ? '#2d8a5e' : 'white', color: page === p ? 'white' : '#2d8a5e', cursor: 'pointer', fontWeight: 600 }}
                  >
                    {p}
                  </button>
                </div>
            ...
            <button
              ...
              style={{ padding: '8px 14px', borderRadius: '8px', border: '1.5px solid #c8e6c9', backgroundColor: 'white', cursor: page === totalPages ? 'not-allowed' : 'pointer', color: page === totalPages ? '#aaa' : '#2d8a5e', fontWeight: 600 }}
            >
              다음
            </button>
          </div>
      ...
      {showModal && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <h2 style={{ fontSize: '18px', fontWeight: 700, color: '#2d8a5e' }}>영양제 추가</h2>
              <button
            ...
            <div style={{ display: 'flex', gap: '8px', marginTop: '1.5rem', justifyContent: 'flex-end' }}>
              <button
                onClick={() => setShowModal(false)}
                style={{ padding: '10px 20px', borderRadius: '10px', border: '1.5px solid #c8e6c9', backgroundColor: 'white', color: '#2d8a5e', cursor: 'pointer', fontWeight: 600 }}
              >
                취소
              </button>
              <button
                onClick={handleAdd}
                style={{ padding: '10px 20px', borderRadius: '10px', border: 'none', backgroundColor: '#2d8a5e', color: 'white', cursor: 'pointer', fontWeight: 600 }}
              >
                추가
              </button>
            </div>
