...
import 'package:simcap/services/chatbot_api_service.dart';
...
class _ChatbotScreenState extends State<ChatbotScreen> {
  ...
  Future<Map<String, dynamic>> _buildUserProfileContext() async {
    final prefs = await SharedPreferences.getInstance();
    return {
      'name': prefs.getString('userName') ?? '',
      'age': int.tryParse(prefs.getString('userAge') ?? ''),
      'gender': prefs.getString('userGender') ?? '',
      'goals': prefs.getStringList('selectedGoals') ?? const [],
      'healthConditions': prefs.getStringList('selectedHealth') ?? const [],
      'allergies': prefs.getStringList('selectedAllergies') ?? const [],
      'smokingStatus': prefs.getString('smokingStatus') ?? '',
      'drinkingStatus': prefs.getString('drinkingStatus') ?? '',
      'pregnancyStatus': prefs.getString('pregnancyStatus') ?? '',
    };
  }
  ...
  Future<void> _sendMessage() async {
    ...
    try {
      final userProfile = await _buildUserProfileContext();
      final response = await ChatbotApiService().sendMessage(
        message: text,
      ...
      setState(() {
        ...
        _messages.add({
          'role': 'bot',
          'message':
              '서버 AI 상담 연결에 실패했습니다. 잠시 후 다시 시도해주세요.\n\n'
